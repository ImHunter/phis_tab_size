...
Процедура ОбработатьРезультатыЗагрузкиРазмеров(РезультатыЗагрузкиРазмеров)
	...
	Если РезультатыЗагрузкиРазмеров.ВыгрузитьПлатформой Тогда 
		ОбработатьРезультатыЗагрузкиРазмеровПлатформой();
	Иначе
		ОбработатьРезультатыЗагрузкиРазмеровИзФайла(РезультатыЗагрузкиРазмеров);
	КонецЕсли;
	
КонецПроцедуры    

&НаСервере
Процедура ОбработатьРезультатыЗагрузкиРазмеровИзФайла(РезультатыЗагрузкиРазмеров)  
...
&НаСервере
Процедура ОбработатьРезультатыЗагрузкиРазмеровПлатформой()  
	
	ТаблицаРазмеров = НоваяТаблицаРазмеров();
	//ДанныеРазмеров = Вычислить("ПолучитьРазмерДанныхБазыДанных()");
	СтруктураХранения = ПолучитьСтруктуруХраненияБазыДанных(, Истина);
	
	ДопустимыеНазначения = Новый Структура;
	ДопустимыеНазначения.Вставить("Основная");
	ДопустимыеНазначения.Вставить("Константы");
	
	Для Каждого ЭлементХранения Из СтруктураХранения Цикл 
		
		Если ПустаяСтрока(ЭлементХранения.Метаданные) Тогда 
			Продолжить;
		КонецЕсли;
		
		Если Не ДопустимыеНазначения.Свойство(ЭлементХранения.Назначение) Тогда 
			Продолжить;
		КонецЕсли;
		
		ОбъектМетаданных = Метаданные.НайтиПоПолномуИмени(ЭлементХранения.Метаданные);
		МассивДляРазмеров = Новый Массив;
		МассивДляРазмеров.Добавить(ОбъектМетаданных);
		// Попытка-исключение для отладки
		Попытка
			Размер = Вычислить("ПолучитьРазмерДанныхБазыДанных(, МассивДляРазмеров)") / 1024 / 1024;
		Исключение
			Сообщить("Объект метаданных: " + ОбъектМетаданных + "
			|ИмяТаблицыХранения: " + ЭлементХранения.ИмяТаблицыХранения + "
			|ИмяТаблицы: " + ЭлементХранения.ИмяТаблицы + "
			|Метаданные: " + ЭлементХранения.Метаданные + "
			|Назначение: " + ЭлементХранения.Назначение);
			ВызватьИсключение;
		КонецПопытки;
		
		СтрокаРазмеров = ТаблицаРазмеров.Добавить();
		СтрокаРазмеров.table_name = ЭлементХранения.ИмяТаблицыХранения;
		СтрокаРазмеров.used_size_int = Размер;
